...
{
	name = "[L4D2] OutpostMod Bonus (Scoremod+)",
	author = "Visor, Sir, Yird",
...
Action:CmdBonus(client, args)
{
	...
	else
	{
		...
		PrintToChat(client, "%s\x01R\x04#%i\x01 Bonus: \x05%d\x01 <\x03%.1f%%\x01> [Health: \x05%d\x01 | Damage: \x05%d\x01 | Pills: \x05%d\x01]", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus), RoundToFloor(fHealthBonus), RoundToFloor(fDamageBonus), RoundToFloor(fPillsBonus));
